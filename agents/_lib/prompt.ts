...
function buildSandboxPreview() {
  return [
    ...
    'A user message that is only a masked API key, for example "API Key: sk-••••••••wxyz" or "API Key： sk-••••••••wxyz", means the host already wrote that key into .env. Call start_preview once with restart:true and do nothing else. Do not read .env, do not write it, and do not quote the key.',
